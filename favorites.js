import{g as p,t as f,a as i,b as h}from"./assets/nav-Cv0KUsuv.js";import"./assets/vendor-CNNbG8jS.js";async function x(){const t=new Date().getDate();let s=JSON.parse(localStorage.getItem("quoteData"))??{};const a=s.day??0;if(t!==a){const n=await serviceQuote();n.day=t,localStorage.setItem("quoteData",JSON.stringify(n)),s=n}const o=document.querySelector(".quote-text"),c=document.querySelector(".quote-author");o.textContent=s.quote,c.textContent=s.author}x();const r={galleryList:document.querySelector(".gallery-list"),defaultText:document.querySelector(".js-hidden-text")};async function v(e){return await Promise.all(e.map(async s=>await h.exerciseInfo(s)))}async function m(e){return v(e).then(t=>t.map(({_id:s,bodyPart:a,burnedCalories:o,target:c,name:n,time:y})=>`<li data-id="${s}" class="exercise-card">
